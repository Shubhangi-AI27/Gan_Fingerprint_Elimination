"""
StyleGAN2-ADA Image Generator - Portable Version
Generates 2000 high-resolution faces (1024x1024) using pre-trained FFHQ weights.
Works seamlessly on Kaggle, Google Colab, or Local Machines.
"""
...
# ── 1. AUTO-DETECT PLATFORM & SET BASE PATHS ──────────────────────────
BASE_DIR = os.getcwd()
...
CHECKPOINT_PATH = os.path.join(BASE_DIR, 'ffhq.pkl')

# Clone Repo if missing
if not os.path.exists(os.path.join(BASE_DIR, REPO_NAME)):
    print("Cloning StyleGAN2-ADA repository...")
    # Using python-based cloning or system execution depending on environment
...
for folder in ['dnnlib', 'torch_utils']:
    src = os.path.join(BASE_DIR, REPO_NAME, folder)
...
# Bypassing specialized CUDA kernels for Kaggle/Colab cross-compatibility
...
OUTPUT_DIR = os.path.join(BASE_DIR, 'batch_2000_highres')
ZIP_NAME = os.path.join(BASE_DIR, 'shubhangi_dataset_2000')
    ...
    for i in tqdm(range(TOTAL_IMAGES)):
        ...
        if i == 0:
            ...
            try:
                ...
                # Fallback if run inside a pure terminal instead of a notebook
                pass
...
try:
    display(FileLink(f'{os.path.basename(ZIP_NAME)}.zip'))
except NameError:
    print(f"ZIP file saved to {ZIP_NAME}.zip")
